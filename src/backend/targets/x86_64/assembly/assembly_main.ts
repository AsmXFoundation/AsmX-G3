...
interface impl_assembly_fn_base {
  fn_name: string;
  fn_body: Buffer;
  fn_signature: string;
  fn_locals: Array<any_variable>;
  fn_locals_size: number;
}

// any fn
class impl_assembly_fn_base implements impl_assembly_fn_base {
  constructor() {
  ...
  gen_allocate_local_variables(): Buffer {
  ...
  gen_free_local_variables(): Buffer {
    let buf = Buffer.alloc(0);
    const locals_size = this.fn_locals.map(l => l.size).reduce((a, b) => a + b, 0);
  ...
  create_allocate_local_variable(name: string, type: string, size: number) {
    this.fn_locals.push({ name, type, size });
    this.fn_locals_size += size;
  }

  gen_body(body: Buffer) {
    this.fn_body = body;
  }
}
...
export {
  ...
  impl_assembly_fn_base,
  impl_assembly_fn_call,
  impl_assembly_fn_pass_args_via_abi,
  impl_assembly_fn_main
}
