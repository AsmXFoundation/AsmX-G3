...
import { AssemblyInstructionDescriptorTable } from "./tbl.cjs";
import {
  OperandSize,
...
const idt = AssemblyInstructionDescriptorTable;
const providetypes = HardwareMachineFactoryParserOpernadType;
const compiler = new HardwareMachineFactory();

idt.defineInstructions();
compiler.setInstructionSet(idt.getInstructionSet() as any);
console.log("ISA AMD64 sizeof: ", compiler.size);

class UnitTest {
  public static test(message: string, actual: Buffer, expected: Buffer): void {
...
try {
  ...
  compiler.compile('add', [providetypes.parseReg('cl') as Operand, providetypes.parseReg('al') as Operand]);
  ...
  compiler.compile('add', [providetypes.parseReg('rbx') as Operand, providetypes.parseReg('rcx') as Operand]);
  ...
  compiler.compile('add', [providetypes.parseReg('r10b') as Operand, providetypes.parseReg('r11b') as Operand]); // REX needed for R10/R11
  UnitTest.test('ADD r10b, r11b', compiler.getSource(), Buffer.from([0x45, 0x00, 0xDA]));
  compiler.clearSource();

  // ADD AL, 5 (Opcode 04 ib) -> 04 05
  compiler.compile('add', [providetypes.parseReg('al') as Operand, providetypes.parseImm(5)]);
  UnitTest.test('ADD AL, 5', compiler.getSource(), Buffer.from([0x04, 0x05]));
  compiler.clearSource();

  // ADD EAX, 12345678h (Opcode 05 id) -> 05 78 56 34 12
  compiler.compile('add', [providetypes.parseReg('eax') as Operand, providetypes.parseImm(0x12345678)]);
  ...
  compiler.compile('add', [providetypes.parseReg('rax') as Operand, providetypes.parseImm(0x12345678)]);
  ...
  compiler.compile('add', [providetypes.parseMem({ size: OperandSize.Byte, base: 'rbx' }), providetypes.parseImm(0x10)]);
  ...
  compiler.compile('add', [
    providetypes.parseMem({ size: OperandSize.Qword, base: 'rbx', index: 'rcx', scale: 4, displacement: 0x100 }),
  ...
  // Imm8 = 01
  // Result: 49 83 C7 01
  compiler.compile('add', [providetypes.parseReg('r15') as Operand, providetypes.parseImm(1)]);
  ...
  compiler.compile('or', [providetypes.parseReg('eax') as Operand, providetypes.parseReg('ebx') as Operand]);
  UnitTest.test('OR EAX, EBX', compiler.getSource(), Buffer.from([0x09, 0xD8]));
  compiler.clearSource();

  // ADD eax, edx
  // Result: 01 D0
  compiler.compile('add', [providetypes.parseReg('eax') as Operand, providetypes.parseReg('edx') as Operand]);
  UnitTest.test('ADD EAX, EDX', compiler.getSource(), Buffer.from([0x01, 0xD0]));
  compiler.clearSource();

  // ADD rax, r15
  // Result: 4C 01 F8
  compiler.compile('add', [providetypes.parseReg('rax') as Operand, providetypes.parseReg('r15') as Operand]);
  ...
  compiler.compile('and', [providetypes.parseReg('eax') as Operand, providetypes.parseReg('ebx') as Operand]);
  ...
  compiler.compile('sub', [providetypes.parseReg('eax') as Operand, providetypes.parseReg('ebx') as Operand]);
  ...
  compiler.compile('xor', [providetypes.parseReg('eax') as Operand, providetypes.parseReg('ebx') as Operand]);
  ...
  compiler.compile('cmp', [providetypes.parseReg('eax') as Operand, providetypes.parseReg('ebx') as Operand]);
  ...
  compiler.compile('cmp', [providetypes.parseReg('ecx') as Operand, providetypes.parseReg('r8d') as Operand]);
  ...
  compiler.compile('mov', [providetypes.parseReg('dl') as Operand, providetypes.parseReg('al') as Operand]);
  UnitTest.test('MOV DL, AL', compiler.getSource(), Buffer.from([0x88, 0xc2]));
  compiler.clearSource();

  // 89 /r: MOV r/m32, r32 -> MOV EDX, ECX
  compiler.compile('mov', [providetypes.parseReg('edx') as Operand, providetypes.parseReg('ecx') as Operand]);
  UnitTest.test('MOV EDX, ECX', compiler.getSource(), Buffer.from([0x89, 0xca]));
  compiler.clearSource();
  
  // 89 /r: MOV r/m64, r64 -> MOV RBX, RCX
  compiler.compile('mov', [providetypes.parseReg('rbx') as Operand, providetypes.parseReg('rcx') as Operand]);
  UnitTest.test('MOV RBX, RCX', compiler.getSource(), Buffer.from([0x48, 0x89, 0xcb]));
  compiler.clearSource();

  // 88 /r: MOV r8, r/m8 -> MOV AL, CL
  compiler.compile('mov', [providetypes.parseReg('al') as Operand, providetypes.parseReg('cl') as Operand]);
  UnitTest.test('MOV AL, CL', compiler.getSource(), Buffer.from([0x88, 0xc8]));
  compiler.clearSource();
  
  // 89 /r: MOV r32, r/m32 -> MOV ECX, EDX
  compiler.compile('mov', [providetypes.parseReg('ecx') as Operand, providetypes.parseReg('edx') as Operand]);
  UnitTest.test('MOV ECX, EDX', compiler.getSource(), Buffer.from([0x89, 0xd1]));
  compiler.clearSource();
  
  // 89 /r: MOV r64, r/m64 -> MOV RCX, RDX
  compiler.compile('mov', [providetypes.parseReg('rcx') as Operand, providetypes.parseReg('rdx') as Operand]);
  UnitTest.test('MOV RCX, RDX', compiler.getSource(), Buffer.from([0x48, 0x89, 0xd1]));
  compiler.clearSource();

  // 8B /r: MOV r32, [mem] -> MOV ebx, dword ptr [eax+ecx*2+10h]
  compiler.compile('mov', [providetypes.parseReg('ebx') as Operand, providetypes.parseMem({ size: OperandSize.Dword, base: 'eax', index: 'ecx', scale: 2, displacement: 0x10 })]);
  ...
  compiler.compile('mov', [providetypes.parseReg('al') as Operand, providetypes.parseImm(0x12)]);
  UnitTest.test('MOV AL, 0x12', compiler.getSource(), Buffer.from([0xb0, 0x12]));
  compiler.clearSource();
  
  // B0+rb ib: MOV r8, imm8 -> MOV R15B, 0x12 (needs REX.B)
  compiler.compile('mov', [providetypes.parseReg('r15b') as Operand, providetypes.parseImm(0x12)]);
  UnitTest.test('MOV R15B, 0x12', compiler.getSource(), Buffer.from([0x41, 0xb7, 0x12]));
  compiler.clearSource();
  
  // B8+rw iw: MOV r16, imm16 -> MOV AX, 0x1234
  compiler.compile('mov', [providetypes.parseReg('ax') as Operand, providetypes.parseImm(0x1234)]);
  UnitTest.test('MOV AX, 0x1234', compiler.getSource(), Buffer.from([0x66, 0xb8, 0x34, 0x12]));
  compiler.clearSource();
  
  // B8+rd id: MOV r32, imm32 -> MOV EAX, 0x12345678
  compiler.compile('mov', [providetypes.parseReg('eax') as Operand, providetypes.parseImm(0x12345678)]);
  ...
  compiler.compile('mov', [providetypes.parseMem({ size: OperandSize.Byte, base: 'rax' }), providetypes.parseImm(0x12)]);
  ...
  compiler.compile('mov', [providetypes.parseMem({ size: OperandSize.Word, base: 'rcx' }), providetypes.parseImm(0x1234)]);
  ...
  compiler.compile('mov', [providetypes.parseMem({ size: OperandSize.Dword, base: 'rdx' }), providetypes.parseImm(0x12345678)]);
  ...
  compiler.compile('mov', [providetypes.parseMem({ size: OperandSize.Qword, base: 'rbx' }), providetypes.parseImm(0x12345678)]);
  ...
  compiler.compile('mov', [providetypes.parseMem({ size: OperandSize.Qword, base: 'r15' }), providetypes.parseImm(0x1234)]);
  ...
  compiler.compile('mov', [providetypes.parseReg('rax') as Operand, providetypes.parseImm(1)]);
  UnitTest.test('MOV rax, 1', compiler.getSource(), Buffer.from([0x48, 0xc7, 0xc0, 0x01, 0x00, 0x00, 0x00]));
  compiler.clearSource();

  // C7 /0 ib: MOV r64, imm64 -> MOV rdi, 1
  compiler.compile('mov', [providetypes.parseReg('rdi') as Operand, providetypes.parseImm(1)]);
  ...
  compiler.compile('mov', [providetypes.parseReg('rdx') as Operand, providetypes.parseImm(13)]);
