...
class Core {
  run(src, parameters) {
    ...
    if (fs.existsSync(src)) {
      ...
      // console.log("Compiler parameters: ", parameters);
      Server.journal.process("Compiling assembly ...");

      const outname = Reflect.ownKeys(parameters).includes('objname') ? parameters.objname : 'a.out';
      // console.log(outname);
      new CompilerDriver(ast, outname);
      
      Server.journal.success(`Assembly compiled`);
