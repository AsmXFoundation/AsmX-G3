...
const enum DynamicTagEnumerate {
  DT_NULL,
  DT_NEEDED,
  DT_PLTRELSZ,
  DT_PLTGOT,
...
enum RELOCATION_X86_64_TYPE_ENUMERATOR {
  R_X86_64_NONE,
  R_X86_64_64,
  R_X86_64_PC32,
  R_X86_64_GOT32,
...
class HardwareCompilerJournal {
  static verbose_lvl = 0;

  static success(verbose_level: number, message: string) {
    if (verbose_level <= this.verbose_lvl) {
      console.log(`[32mhwc driver:\x1b[0m ${message}\x1b[0m`);
    }
  }

  static warn(verbose_level: number, message: string) {
    if (verbose_level <= this.verbose_lvl) {
      console.log(`[33mhwc driver:\x1b[0m ${message}\x1b[0m`);
    }
  }

  static error(verbose_level: number, message: string) {
    if (verbose_level <= this.verbose_lvl) {
...
class HardwareCompiler {
  ...
  private sectionMap: Map<string, Section> = new Map();
  private programHeaders: ProgramHeader[] = [];
  private entryPoint: number = 0;
  private baseAddress: number = 0x400000;
  private interpreter: string = '/lib64/ld-linux-x86-64.so.2';
  private sectionsRequired: Set<string> = new Set(['', '.shstrtab']);
  private dataAddedTo: Set<string> = new Set();
  private dynamicLinkEnabled: boolean = false;
  private dynamicSymbols: Map<string, SymbolInfo> = new Map();
  private localSymbols: Map<string, SymbolInfo> = new Map();
  private dynSymEntries: { name: string, type: number, binding?: number }[] = [];
  private neededLibraries: string[] = [];
  private sectionHeadersOffset: number = 0;
  private relocations: Relocation[] = [];
  private relaPltOrder: string[] = [];
  ...
  private addSection(
    name: string, type: number, flags: number, align: number, entrySize: number = 0, link: number = 0, info: number = 0
  ): Section {
  ...
  public getSectionPub(name: string): Section {
    return this.getSection(name);
  }

  private requireSection(name: string): void {
  ...
  private internalEmitBytes(sectionName: string, data: Buffer): void {
  ...
  public addSymbol(name: string, sectionName: string, offset: number, size: number, type: number, binding: number = STB_LOCAL): void {
    ...
    if (binding === STB_LOCAL) {
      ...
      this.requireSection(SectionName.SYMB_TABLE);
      this.requireSection(SectionName.STR_TABLE);
    } else {
  ...
  public addImmMutableData(data: Buffer, type: string, symbolName?: string): number {
    this.requireSection(SectionName.RODATA);
    const section = this.getSection(SectionName.RODATA);
    const length = section.data.length;
    const align = alignTo(length, section.align);

    if (align > length) {
      this.internalEmitBytes(SectionName.RODATA, Buffer.alloc(align - length));
    }

    const offset = section.data.length;
    this.internalEmitBytes(SectionName.RODATA, data);

    if (symbolName) {
      this.addSymbol(symbolName, SectionName.RODATA, offset, data.length, STT_OBJECT, STB_LOCAL);
    }
  ...
  private enableDynamicLinking(): void {
    ...
    const requireSections = [
      ...
      SectionName.DYNAMIC, SectionName.DYN_SYMB, SectionName.DYN_STR,
      SectionName.PLT, SectionName.GOT_PLT,
      SectionName.RELA_PLT, SectionName.RELA_DYN,
      SectionName.HASH
    ];
    ...
    this.getSection(SectionName.INTERPRETER).data = Buffer.from(this.interpreter + '\0');
    this.dataAddedTo.add(SectionName.INTERPRETER);
  ...
  public async requireSymbol(name: string, libPrefix?: string, type: number = STT_FUNC): Promise<void> {
  ...
  public addInstruction(buffer: Buffer, reloc?: Relocation): void {
    this.requireSection(SectionName.TEXT);
    this.internalEmitBytes(SectionName.TEXT, buffer); // Add the instruction bytes
    if (reloc) {
  ...
  public addCode(code: Buffer, relocs: Relocation[] = []): void {
    HardwareCompilerJournal.warn(3, "Self.addCode is deprecated for step-by-step generation. Use addInstruction.");
    this.requireSection(SectionName.TEXT);
    this.internalEmitBytes(SectionName.TEXT, code);
    this.relocations.push(...relocs);
  }

  public generateSymbolReference(name: string, type: 'call' | 'data' | 'address'): { buffer: Buffer, reloc: Relocation } {
    ...
    const textSection = this.getSection(SectionName.TEXT);
    ...
    let buffer: Buffer;
    let reloc: Relocation | null = null;
    let relocOffset = -1;
  ...
  private buildFinalDynStrData(): void {
    if (!this.sectionsRequired.has(SectionName.DYN_STR)) {
      return;
    }

    HardwareCompilerJournal.success(1, "Building .dynstr...");
    const dynstr = this.getSection(SectionName.DYN_STR);
    let finalStr = '\0';
    ...
    this.dataAddedTo.add(SectionName.DYN_STR);
  }

  private buildFinalShstrtabData(): void {
    if (!this.sectionsRequired.has(SectionName.SHSTR_TABLE)) {
      return;
    }
    
    HardwareCompilerJournal.success(1, "Building .shstrtab...");
    const shstrtab = this.getSection(SectionName.SHSTR_TABLE);
    let finalStr='\0';
    ...
    this.dataAddedTo.add(SectionName.SHSTR_TABLE);
  }
  
  private buildFinalDynSymData(): void {
    if (!this.sectionsRequired.has(SectionName.DYN_SYMB)) {
      return;
    }

    HardwareCompilerJournal.success(1, "Building .dynsym...");
    const dynsym = this.getSection(SectionName.DYN_SYMB);
    const dynstr = this.getSection(SectionName.DYN_STR);
    ...
    const finalEntries: Buffer[] = [Buffer.alloc(24)];
    ...
    this.dynSymEntries.forEach((symEntryInfo) => {
      ...
      entry.writeUInt8(0, 5); entry.writeUInt16LE(STN_UNDEF, 6);
    ...
    dynsym.link = this.getSectionIndex(SectionName.DYN_STR);
    dynsym.info = 1; /* Index of first global */
    this.dataAddedTo.add(SectionName.DYN_SYMB);
  }

  private buildFinalSymtabData(): void {
    ...
    if (!this.sectionsRequired.has(SectionName.SYMB_TABLE)) {
      ...
      this.getSection(SectionName.SYMB_TABLE).size = 0;
      this.getSection(SectionName.STR_TABLE).size = 0;
      return;
    }

    HardwareCompilerJournal.success(1, "Building .symtab and .strtab...");
    const symtab = this.getSection(SectionName.SYMB_TABLE);
    const strtab = this.getSection(SectionName.STR_TABLE);

    // Initialize buffers and counters
    const finalEntries: Buffer[] = [Buffer.alloc(24)]; // Start with the mandatory NULL symbol entry
    ...
    this.sections.forEach((sec, idx) => {
      if (sec.name) sectionIndices.set(sec.name, idx);
    });
    ...
    if (symtab.data.length > 24) {
      symtab.link = this.getSectionIndex(SectionName.STR_TABLE); // Link .symtab to its string table .strtab
      ...
      this.dataAddedTo.add(SectionName.SYMB_TABLE);
      this.dataAddedTo.add(SectionName.STR_TABLE);
    }
  }

  private buildFinalPltGotData(): void {
    if (!this.sectionsRequired.has(SectionName.PLT)) {
      return;
    }

    HardwareCompilerJournal.success(1, "Building PLT/GOT buffers...");
    const plt = this.getSection(SectionName.PLT);
    const gotplt = this.getSection(SectionName.GOT_PLT);
    ...
    if (pltSize > 16 /* Dont add if only PLT0 */) {
      this.dataAddedTo.add(SectionName.PLT);
    }

    if(gotPltSize > 24) {
      this.dataAddedTo.add(SectionName.GOT_PLT);
    }

    gotplt.link = this.sectionsRequired.has(SectionName.DYNAMIC) ? this.getSectionIndex(SectionName.DYNAMIC) : 0;
  }

  private buildFinalRelaData(): void {
    HardwareCompilerJournal.success(1, "Building/Recalculating relocation sections (.rela.dyn, .rela.plt)...");
    const relaDyn = this.getSection(SectionName.RELA_DYN);
    const relaPlt = this.getSection(SectionName.RELA_PLT);
    const gotPltSection = this.getSection(SectionName.GOT_PLT);

    const relaDynEntries: Buffer[] = [];
    const relaPltEntries: Buffer[] = [];
    ...
    this.relocations.forEach((reloc) => {
      ...
      let targetRelocSection: 'plt' | 'dyn' | null = null;
      ...
      let r_offset: bigint; // The address field for the RELA entry
    ...
    if (relaDyn.size > 0) {
      relaDyn.link = this.getSectionIndex(SectionName.DYN_SYMB);
      this.dataAddedTo.add(SectionName.RELA_DYN);
  
      if (!this.sectionsRequired.has(SectionName.RELA_DYN)) {
        this.requireSection(SectionName.RELA_DYN);
      }
    } else {
      relaDyn.size = 0;
    ...
    if (relaPlt.size > 0) {
      relaPlt.link = this.getSectionIndex(SectionName.DYN_SYMB);
      this.dataAddedTo.add(SectionName.RELA_PLT);

      if (!this.sectionsRequired.has(SectionName.RELA_PLT)) {
        this.requireSection(SectionName.RELA_PLT);
      }
    } else {
      relaPlt.size = 0;
  ...
  private buildFinalDynamicData(): void {
    if (!this.sectionsRequired.has(SectionName.DYNAMIC)) {
      return;
    }

    HardwareCompilerJournal.success(1, "Allocating .dynamic buffer...");
    const dynamic = this.getSection(SectionName.DYNAMIC);
    const gotPlt = this.getSection(SectionName.GOT_PLT);
    let entryCount = 1;
    entryCount += this.neededLibraries.length;

    if (this.sectionsRequired.has(SectionName.HASH)) {
      entryCount++;
    }
    ...
    const relaDynEntriesCount = this.getSection(SectionName.RELA_DYN).data.length / 24;
    const relaPltEntriesCount = this.getSection(SectionName.RELA_PLT).data.length / 24;
    ...
    dynamic.link = this.getSectionIndex(SectionName.DYN_STR);
    this.dataAddedTo.add(SectionName.DYNAMIC);
    HardwareCompilerJournal.success(1, `.dynamic final entry count: ${entryCount}, allocated size: ${dynamicBufferSize}`);
  }

  private buildFinalHashData(): void {
    if (!this.sectionsRequired.has(SectionName.HASH)) {
      return;
    }

    HardwareCompilerJournal.success(1, "Building .hash...");
    const hashSection = this.getSection(SectionName.HASH);
    ...
    hashSection.link = this.getSectionIndex(SectionName.DYN_SYMB);
    this.dataAddedTo.add(SectionName.HASH);
  }
  ...
  private finalizeLayoutAndHeaders(pageAlign: number = 0x1000): void {
    ...
    if (this.sectionsRequired.has(SectionName.INTERPRETER)) {
      phdrCount++;
    }
    ...
    if (needsRxLoad) phdrCount++;
    if (needsRwLoad) phdrCount++;
    ...
    if (this.sectionsRequired.has(SectionName.INTERPRETER)) {
      const interpSection = this.getSection(SectionName.INTERPRETER);
      this.programHeaders.push({
        type: PT_INTERP,
        flags: PF_R,
    ...
    if (this.sectionsRequired.has(SectionName.DYNAMIC)) {
      const dynamicSection = this.getSection(SectionName.DYNAMIC);
    ...
    this.entryPoint = this.sectionsRequired.has(SectionName.TEXT) ? this.getSection(SectionName.TEXT).address : this.baseAddress;
  ...
  private patchPltGotData(): void {
    ...
    plt.data.writeUInt8(0xff, 6); plt.data.writeUInt8(0x25, 7);
  ...
  private patchDynamicData(): void {
    if (!this.dynamicLinkEnabled || !this.sectionsRequired.has(SectionName.DYNAMIC)) {
      return;
    }
    ...
    const gotplt = this.getSection(SectionName.GOT_PLT);
    const relaDyn = this.getSection(SectionName.RELA_DYN);
    const hashSec = this.getSection(SectionName.HASH);
    ...
    const tryWriteEntry = (tag: number, value: bigint) => {
    ...
    this.neededLibraries.forEach(libName => {
      ...
      tryWriteEntry(DynamicTagEnumerate.DT_NEEDED, BigInt(strOffset));
    });
    
    if (this.sectionsRequired.has(SectionName.HASH)) {
      tryWriteEntry(DynamicTagEnumerate.DT_HASH, BigInt(hashSec.address));
    }
    
    if (this.sectionsRequired.has(SectionName.DYN_STR)) {
      tryWriteEntry(DynamicTagEnumerate.DT_STRTAB, BigInt(dynstr.address));
      tryWriteEntry(DynamicTagEnumerate.DT_STRSZ, BigInt(dynstr.size));
    }
    
    if (this.sectionsRequired.has(SectionName.DYN_SYMB)) {
      tryWriteEntry(DynamicTagEnumerate.DT_SYMTAB, BigInt(dynsym.address));
      tryWriteEntry(DynamicTagEnumerate.DT_SYMENT, BigInt(dynsym.entrySize));
    } 
    
    if (relaDyn.size > 0) {
      tryWriteEntry(DynamicTagEnumerate.DT_RELA, BigInt(relaDyn.address));
      tryWriteEntry(DynamicTagEnumerate.DT_RELASZ, BigInt(relaDyn.size));
      tryWriteEntry(DynamicTagEnumerate.DT_RELAENT, BigInt(24));
    }
    
    if (relaplt.size > 0) {
      tryWriteEntry(DynamicTagEnumerate.DT_PLTRELSZ, BigInt(relaplt.size));
      tryWriteEntry(DynamicTagEnumerate.DT_PLTREL, BigInt(DynamicTagEnumerate.DT_RELA));
      tryWriteEntry(DynamicTagEnumerate.DT_JMPREL, BigInt(relaplt.address));
    }
    
    if(gotplt.size > 0){
      tryWriteEntry(DynamicTagEnumerate.DT_PLTGOT, BigInt(gotplt.address));
    } 

    if(offset + 16 <= dynamic.data.length) {
      tryWriteEntry(DynamicTagEnumerate.DT_NULL, BigInt(0));
    } else {
  ...
  private patchTextRelocations(): void {
    if (!this.sectionsRequired.has(SectionName.TEXT)) {
      return;
    }
    ...
    const pltSection = this.sectionsRequired.has(SectionName.PLT) ? this.getSection(SectionName.PLT) : null;
    const gotPltSection = this.sectionsRequired.has(SectionName.GOT_PLT) ? this.getSection(SectionName.GOT_PLT) : null;

    this.relocations.forEach(reloc => {
      if (reloc.section !== SectionName.TEXT) {
      ...
      switch (reloc.type) {
        ...
        case RELOCATION_X86_64_TYPE_ENUMERATOR.R_X86_64_GOTPCREL:
          ...
          let gotEntryOffset: number | undefined;
  ...
  private updateInitialSectionSizes(): void {
    ...
    this.sections.forEach(section => {
      ...
      const isPreSized = section.name === SectionName.PLT || section.name === SectionName.GOT_PLT; // Sections with explicit sizes set early
  ...
  public compile(outputPath: string): void {
    ...
    this.sections.forEach(section => { // Finalize sizes based on generated data
      if (this.dataAddedTo.has(section.name)) {
        // Use actual data length for sections populated by builders
        if (section.data.length !== section.size && section.name !== SectionName.PLT && section.name !== SectionName.GOT_PLT) {  // Don't override PLT/GOT size here
          section.size = section.data.length;
        }
      ...
      if (section.size > 0 && section.data.length < section.size && (section.name === SectionName.PLT || section.name === SectionName.GOT_PLT) ) {
      ...
      } else if (section.size > 0 && section.data.length > section.size && (section.name === SectionName.PLT || section.name === SectionName.GOT_PLT)) {
    ...
    const shstrndx = this.getSectionIndex(SectionName.SHSTR_TABLE);
    ...
    const shstrtabSection = this.getSection(SectionName.SHSTR_TABLE);
    const shstrOffsets = new Map<string, number>();
    ...
    writeFileSync(outputPath, Buffer.concat(fileContent));
    chmodSync(outputPath, 0o755);
    HardwareCompilerJournal.success(1, `ELF file ${outputPath} generated`);
  }

  public addRelocations(...relocs: Relocation[]): void {
    if (!relocs) return;
    this.relocations.push(...relocs);
  }
}
